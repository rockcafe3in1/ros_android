...
if [[ $samples -eq 1 && "$user_workspace" != "" ]];then
    ...
    if [[ $debugging -eq 1 ]];then
        run_cmd build_catkin_workspace -w $user_workspace -p $prefix -b Debug -v $verbose
    else
        run_cmd build_catkin_workspace -w $user_workspace -p $prefix -b Release -v $verbose
    fi
fi
